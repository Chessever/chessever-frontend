-- For You top games: prefer the freshest event section before rating.
--
-- The previous ranking selected one latest round per rating category, then
-- ordered the final event-level result by avg_elo before round recency. That
-- let older high-rated tiebreak rows occupy the card preview while a newer
-- lower-rated Swiss/final section existed in the same group broadcast.
--
-- Keep live rounds ahead of fallback rounds, but for fallback previews rank by
-- source_round_time before category_avg_elo so completed event cards represent
-- the latest actual section first.
