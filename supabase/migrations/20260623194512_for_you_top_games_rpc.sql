create index if not exists idx_games_tour_top_rating
on public.games (
  tour_id,
  player_max_rating desc nulls last,
  board_nr asc nulls last,
...
as $$
  ...
  cross join lateral (
    ...
    order by
      per_tour.player_max_rating desc nulls last,
      per_tour.board_nr asc nulls last,
      per_tour.date_start desc nulls last,
      per_tour.last_move_time desc nulls last,
      per_tour.id asc
    limit p.board_count
  ) top_games
  order by
