...
as $$
  ...
  cross join params p
  cross join lateral (
    select max(t.avg_elo) as category_avg_elo
    from public.tours t
  ...
  cross join lateral (
    ...
    where t.group_broadcast_id = re.event_id
      and t.avg_elo is not distinct from selected_category.category_avg_elo
      and exists (
        select 1
        from public.games g
        where g.round_id = r.id
  ...
  cross join lateral (
    ...
    where t.group_broadcast_id = re.event_id
      ...
      and (
        (
          latest_round.round_slug is not null
          and nullif(r.slug, '') = latest_round.round_slug
          and coalesce(r.starts_at, r.created_at)
            between latest_round.round_time - interval '30 minutes'
                and latest_round.round_time + interval '30 minutes'
        )
        or (
          latest_round.round_slug is null
          and latest_round.round_name is not null
          and nullif(r.name, '') = latest_round.round_name
          and coalesce(r.starts_at, r.created_at)
            between latest_round.round_time - interval '30 minutes'
                and latest_round.round_time + interval '30 minutes'
        )
        or (
          latest_round.round_slug is null
          and latest_round.round_name is null
          and r.id = latest_round.round_id
        )
      )
    order by
...
create or replace function public.get_for_you_favorite_player_counts(
  p_event_ids text[],
  p_fide_ids bigint[]
)
returns table (
  event_id text,
  favorite_player_count integer,
  fide_ids bigint[]
)
language sql
stable
...
as $$
  ...
  favorite_ids as (
    select distinct fide_id
    from unnest(coalesce(p_fide_ids, '{}'::bigint[])) as fide_id
    where fide_id is not null and fide_id > 0
  ),
  event_matches as (
    select
      re.event_id,
      favorite_ids.fide_id
    from requested_events re
    join public.tours t on t.group_broadcast_id = re.event_id
    join lateral jsonb_array_elements(coalesce(t.players, '[]'::jsonb)) as player(data) on true
    join favorite_ids
      on player.data->>'fideId' ~ '^[0-9]+$'
     and (player.data->>'fideId')::bigint = favorite_ids.fide_id
    group by re.event_id, favorite_ids.fide_id
  )
  select
    re.event_id,
    count(event_matches.fide_id)::integer as favorite_player_count,
    coalesce(
      array_agg(event_matches.fide_id order by event_matches.fide_id)
        filter (where event_matches.fide_id is not null),
      '{}'::bigint[]
    ) as fide_ids
  from requested_events re
  left join event_matches on event_matches.event_id = re.event_id
  group by re.event_id
  order by re.event_id;
$$;

revoke all on function public.get_for_you_favorite_player_counts(text[], bigint[]) from public;
grant execute on function public.get_for_you_favorite_player_counts(text[], bigint[]) to authenticated;
