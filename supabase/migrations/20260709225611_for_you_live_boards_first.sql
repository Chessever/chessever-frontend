-- For You top games: promote actively playing boards before completed boards.
--
-- Keep this entirely inside the existing batched RPC so the Flutter client
-- continues to receive the same bounded payload and can reuse its existing
-- position/clock streams without client-side sorting or over-fetching.
