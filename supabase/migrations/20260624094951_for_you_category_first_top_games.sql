...
as $$
  ...
  cross join lateral (
    ...
    left join lateral (
      select
        case
          when count(*) >= 2 then avg(rated_player.rating)
          else null
        end as board_avg_elo
      from (
        select nullif((player->>'rating')::numeric, 0) as rating
        from jsonb_array_elements(coalesce(g.players, '[]'::jsonb)) as player
        where player ? 'rating'
          and player->>'rating' ~ '^[0-9]+(\.[0-9]+)?$'
      ) rated_player
      where rated_player.rating is not null
    ) rating on true
    where t.group_broadcast_id = re.event_id
      and t.avg_elo is not distinct from selected_category.category_avg_elo
      and (
    ...
    order by
      rating.board_avg_elo desc nulls last,
  ...
  order by
    top_games.event_id,
    top_games.board_avg_elo desc nulls last,
