  ...
  group('buildGameShareUrl', () {
    ...
    test('returns a deep link for saved analyses of ChessEver Direct games', () {
      final url = buildGameShareUrl(
        ...
        savedAnalysisData: _savedAnalysisData(
          sourceGameId: 'AbCd1234-1x2y3z',
        ),
      );
...
''';
  ...
  group('mergeGameReportAnnotationsForExport / Copy+Share PGN hydrate', () {
    ...
    test('chesseverClassificationName maps every class to product slugs', () {
      expect(
        chesseverClassificationName(GameMoveClassification.brilliant),
        'brilliant',
      );
      expect(
        chesseverClassificationName(GameMoveClassification.goodMove),
        'good_move',
      );
      expect(
        chesseverClassificationName(GameMoveClassification.bestMove),
        'best_move',
      );
      expect(
        chesseverClassificationName(GameMoveClassification.missedWin),
        'missed_win',
      );
      expect(
        chesseverClassificationName(GameMoveClassification.inaccuracy),
        'inaccuracy',
      );
      expect(
        chesseverClassificationName(GameMoveClassification.mistake),
        'mistake',
      );
      expect(
        chesseverClassificationName(GameMoveClassification.blunder),
        'blunder',
      );
      expect(
        chesseverClassificationName(GameMoveClassification.bookMove),
        'book_move',
      );
      // Classic glyphs remain available for import/best-effort only.
      expect(
