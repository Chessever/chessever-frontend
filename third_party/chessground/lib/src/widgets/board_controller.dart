...
class ChessboardController extends ChangeNotifier {
  ...
  // CHESSEVER PATCH (Sentry CHESSEVER-1N2): a detached controller must not crash.
  // The board reads these getters every build(); on phone, reparent/remount
  // timing (Android predictive-back, PiP surface recovery, list/page recycling)
  // can run a build while the controller is momentarily detached, in which case
  // `_x!` throws "Null check operator used on a null value" (fatal in release).
  // Returning kAlwaysCompleteAnimation (a settled, non-ticking animation) is the
  // correct semantic for "no controller attached" and renders the static
  // position with no animation. See third_party/chessground/PATCH.md.
  @internal
