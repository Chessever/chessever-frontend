...
/// Plain models for the Discovery surfaces (Studies + Miniatures).
...
/// Coarse quality band for a study's credibility score (0–100), so readers can
/// classify quality at a glance. The best studies also surface first via the
/// default `score` sort, so these bands and the ordering reinforce each other.
String studyQualityLabel(double score) {
  if (score >= 70) return 'Top';
  if (score >= 45) return 'Great';
  return 'Good';
}
...
class Miniature {
  ...
  /// FIDE 3-letter federation codes (e.g. USA / NOR), used for the flag on the
  /// game card so miniatures render identically to the rest of the app.
  ...
  factory Miniature.fromJson(Map<String, dynamic> json) {
    return Miniature(
      ...
      whiteElo: json['whiteElo'] == null ? null : _parseInt(json['whiteElo']),
      blackElo: json['blackElo'] == null ? null : _parseInt(json['blackElo']),
      whiteFed: json['whiteFed']?.toString(),
      blackFed: json['blackFed']?.toString(),
    );
  }
  ...
  GamesTourModel toGamesTourModel() {
    ...
    return GamesTourModel(
      ...
      whitePlayer: PlayerCard(
        ...
        federation: (whiteFed ?? '').trim(),
        title: ChessTitleUtils.normalize(''),
        rating: whiteElo ?? 0,
        countryCode: (whiteFed ?? '').trim(),
        team: null,
      ),
      blackPlayer: PlayerCard(
        name: (blackName ?? 'Black').trim().isEmpty ? 'Black' : blackName!,
        federation: (blackFed ?? '').trim(),
        title: ChessTitleUtils.normalize(''),
        rating: blackElo ?? 0,
        countryCode: (blackFed ?? '').trim(),
        team: null,
      ),
      whiteTimeDisplay: '--:--',
      blackTimeDisplay: '--:--',
