  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return GestureDetector(
      ...
      child: RefreshIndicator(
        ...
        displacement: 60.h,
        strokeWidth: 3.w,
        child: ref.watch(gamesAppBarProvider).when(
          data: (_) {
            return ref.watch(gamesTourScreenProvider).when(
              data: (data) {
                if (data.gamesTourModels.isEmpty) {
                  return EmptyWidget(
                    title: "No games available yet. Check back soon or set a\nreminder for updates.",
                  );
                }
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (game.gameStatus == GameStatus.ongoing) {
                      ref.watch(gameFenStreamProvider(game.gameId)).whenOrNull(
                        data: (fen) {
                          if (fen != null) {
                    ...
                    return Padding(
                      ...
                      child: isChessBoardVisible
                        ...
                        onPinToggle: (gamesTourModel) async {
                          await ref.read(gamesTourScreenProvider.notifier).togglePinGame(gamesTourModel.gameId);
                        },
                      ),
                    );
                  },
