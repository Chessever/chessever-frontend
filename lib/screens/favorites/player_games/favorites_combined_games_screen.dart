...
class _FavoritesCombinedGamesScreenState
  ...
  List<GamesTourModel> _filterGames(
    ...
    if (_selectedPlayerIds.isNotEmpty) {
      final selectedPlayerNames = favorites
          .where((f) => _selectedPlayerIds.contains(f.id))
          .map((f) => f.playerName.toLowerCase())
          .toSet();

      final selectedFideIds = favorites
          .where((f) =>
              _selectedPlayerIds.contains(f.id) &&
              f.fideId != null &&
              f.fideId!.isNotEmpty)
          .map((f) => f.fideId!)
          .toSet();
      ...
      filtered = filtered.where((game) {
        ...
        final whiteMatches = selectedPlayerNames.any((name) =>
                whiteName.contains(name) || name.contains(whiteName)) ||
            selectedFideIds.contains(whiteFideId);
        final blackMatches = selectedPlayerNames.any((name) =>
                blackName.contains(name) || name.contains(blackName)) ||
