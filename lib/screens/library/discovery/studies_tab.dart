...
/// The Studies tab: our own curated, quality-ranked studies. Games are served
/// and analyzed entirely in-app — the user is never sent to any external site.
class StudiesTab extends ConsumerStatefulWidget {
  const StudiesTab({super.key});

  @override
  ConsumerState<StudiesTab> createState() => _StudiesTabState();
}

class _StudiesTabState extends ConsumerState<StudiesTab> {
  ...
  void initState() {
    super.initState();
    // Reflect any query already set (e.g. seeded from the Discover rail).
    _searchController.text = ref.read(studiesQueryProvider).q;
  }
  ...
  void _onSearchChanged(String value) {
    ...
    _debounce = Timer(const Duration(milliseconds: 300), () {
      if (!mounted) return;
      ref.read(studiesQueryProvider.notifier).setSearch(value.trim());
    });
  }
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: SimpleSearchBar(
                ...
                onOpenFilter: _pickFilters,
                onChanged: _onSearchChanged,
                onCloseTap: () {
                  _debounce?.cancel();
                  _searchController.clear();
                  _searchFocusNode.unfocus();
                  ref.read(studiesQueryProvider.notifier).setSearch('');
                },
              ),
            ),
          ),
