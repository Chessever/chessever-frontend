...
extension GamesLocalStorageEnhancedSearch on GamesLocalStorage {
  Future<EnhancedGameSearchResult> searchGamesWithScoring({
    ...
    try {
      ...
      return searchTournamentGameCatalog(games, query, maxResults: maxResults);
    } catch (e) {
...
/// Search a complete catalog, independently of viewport and expansion state.
/// A null limit returns every match, including boards past the first page.
EnhancedGameSearchResult searchTournamentGameCatalog(
  ...
  final limitedResults =
      maxResults == null ? results : results.take(maxResults).toList();
